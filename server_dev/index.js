...
const writeImage = (data, path) => new Promise((res, rej) => {
    ...
    fs.writeFile(`${__dirname}/../public/uploads/${path}`, data, (err) => {
        if(err){
            rej("image_save_fail");
         }else{
            res(true);
...
const write = (data) => new Promise((res, rej) => {
    if(Object.keys(data).length > 0){ 
        fs.writeFile(`${__dirname}/../public/data/recipes.json`, data, (err) => {
            if(err){
...
const remove = (path) => new Promise((res, rej) => {
    ...
    fs.access(`${__dirname}/../public/uploads/${path}`, err => {
        if(!err){
            fs.unlinkSync(`${__dirname}/../public/uploads/${path}`)
            res(true);
        }else{
...
app.get('/', (req, res) => {
    ...
    fs.readFile(`${__dirname}/../public/data/recipes.json`,(err, data) => {
        if(!err){
            try{ 
